...
def abrir_cookie(url):
        ...
        br.open('http://82.199.155.116:8008/admin/')
        br.select_form(nr=0)
...
def canais_playtvfr():
    canais = eval(abrir_cookie('http://82.199.155.116:8008/canais/playtvfr?action=1'))
    for canal in canais:
	addDir(canal[0].encode('utf-8', 'ignore'),canal[2],'3001',canal[1],len(canais),False)
    xbmc.executebuiltin("Container.SetViewMode(500)")

def play_playtvfr(url):
    m3u8 = abrir_cookie('http://82.199.155.116:8008/canais/playtvfr?action=2&ch=%s' % url)
    xbmcPlayer = xbmc.Player()
    xbmcPlayer.play(m3u8+'|User-agent=')


def canais_tvzune():
    canais = eval(abrir_cookie('http://82.199.155.116:8008/canais/tvzune?action=1'))
    for canal in canais:
	addDir(canal[0],canal[1],'2001',canal[2],len(canais),False)
    xbmc.executebuiltin("Container.SetViewMode(500)")

def play_tvzune(url):
    m3u8 = abrir_cookie('http://82.199.155.116:8008/canais/tvzune?action=2&ch=%s' % url)
    xbmcPlayer = xbmc.Player()
    xbmcPlayer.play(m3u8+'|User-agent=')
...
def listar_filmes(request):
        ...
        filmes = json.loads(abrir_cookie('http://82.199.155.116:8008/filme/filmes?%s' % request))
        print filmes
